...
// @notice: Logging utils which adds to the window.console objct
function interceptConsole() {
  ...
  function processArgs(args) {
    for (let i = 0; i < args.length; i++) {
      ...
      if (isString && arg.indexOf("Slow network is detected.") !== -1)
        return [];
...
function randBadIcon() {
  return ICONS_BAD[randInt(0, ICONS_BAD.length)];
}
function randGoodIcon() {
  return ICONS_GOOD[randInt(0, ICONS_GOOD.length)];
}
